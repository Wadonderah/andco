...
class WhatsAppService {
  ...
  Future<void> initialize() async {
    ...
    try {
      // Test WhatsApp API connection (optional)
      // You can add API validation here if needed

      _isInitialized = true;
      debugPrint('✅ WhatsApp service initialized successfully');

      // Log initialization event if FirebaseService is available
      try {
        // Assuming FirebaseService is imported - you may need to add the import
        // await FirebaseService.instance.logEvent('whatsapp_service_initialized', {});
      } catch (e) {
        // Ignore if FirebaseService is not available
        debugPrint('Note: Firebase logging not available for WhatsApp service');
      }
    } catch (e) {
      debugPrint('❌ Failed to initialize WhatsApp service: $e');
      rethrow;
    }
  }
  ...
  Future<bool> sendTemplateMessage(
      String to, String templateName, Map<String, String> parameters) async {
    try {
      final components = parameters.entries
          .map((entry) => {
                'type': 'text',
                'text': entry.value,
              })
  ...
  Future<bool> sendInteractiveMessage(
      String to, String bodyText, List<WhatsAppButton> buttons) async {
    try {
      final buttonComponents = buttons
          .map((button) => {
                'type': 'reply',
                'reply': {
                  'id': button.id,
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            'Failed to send WhatsApp interactive message: ${response.body}');
        return false;
      }
    } catch (e) {
  ...
  Future<bool> sendLocationMessage(String to, double latitude, double longitude,
  ...
  Future<WhatsAppResponse> handleWebhook(
        ...
        switch (messageType) {
          ...
          default:
            return WhatsAppResponse(
              success: true,
              reply:
                  'Sorry, I can only process text messages, buttons, and locations.',
            );
        }
      }
  ...
  Future<WhatsAppResponse> _handleTextMessage(
      String from, String text, String profileName) async {
  ...
  Future<WhatsAppResponse> _handleBotCommand(
      String from, String command, BotConversation conversation) async {
    switch (command) {
      case '/start':
      ...
      case '/track':
        ...
        await sendTextMessage(from,
            'Please provide your child\'s name or student ID to track their bus.');
        break;

      case '/schedule':
        conversation.state = ConversationState.checkSchedule;
        await sendTextMessage(from,
            'Please provide your child\'s name or route number to check the schedule.');
        break;
      ...
      case '/stop':
        ...
        await sendTextMessage(from,
            'Thank you for using School Bus Tracker! Have a great day! 👋');
        _activeConversations.remove(from);
        break;

      default:
        await sendTextMessage(
            from, 'Unknown command. Type /help for available commands.');
    }

    return WhatsAppResponse(success: true);
  }

  Future<WhatsAppResponse> _handleInitialMessage(
      String from, String text, BotConversation conversation) async {
    if (text.contains('track') ||
        text.contains('bus') ||
        text.contains('location')) {
      conversation.state = ConversationState.trackingBus;
      await sendTextMessage(from,
          'I\'ll help you track the bus! Please provide your child\'s name or student ID.');
    } else if (text.contains('schedule') || text.contains('time')) {
      conversation.state = ConversationState.checkSchedule;
      await sendTextMessage(from,
          'I\'ll check the schedule for you! Please provide your child\'s name or route number.');
    } else if (text.contains('problem') ||
        text.contains('issue') ||
        text.contains('report')) {
      conversation.state = ConversationState.reportIssue;
      await sendTextMessage(from,
          'I\'m sorry to hear about the issue. Please describe what happened.');
    } else {
  ...
  Future<WhatsAppResponse> _handleBusTracking(
      String from, String text, BotConversation conversation) async {
    ...
    await sendLocationMessage(
        from, -1.2921, 36.8219, 'School Bus Location', 'Nairobi, Kenya');
    await sendTextMessage(
        from,
        '🚌 Bus Status Update:\n'
        '• Route: Route A\n'
    ...
    await sendInteractiveMessage(
        from, 'Is there anything else I can help you with?', [
  ...
  Future<WhatsAppResponse> _handleScheduleCheck(
      String from, String text, BotConversation conversation) async {
  ...
  Future<WhatsAppResponse> _handleIssueReport(
      String from, String text, BotConversation conversation) async {
  ...
  Future<WhatsAppResponse> _handleGeneralQuery(
      String from, String text, BotConversation conversation) async {
    // Simple keyword-based responses
    if (text.contains('hello') || text.contains('hi')) {
      await sendTextMessage(from,
          'Hello! How can I assist you with the school bus service today?');
    } else if (text.contains('thank')) {
      await sendTextMessage(
          from, 'You\'re welcome! Is there anything else I can help you with?');
    } else {
      await sendTextMessage(
          from,
  ...
  Future<WhatsAppResponse> _handleButtonReply(
      String from, String buttonId, String profileName) async {
    final conversation = _getOrCreateConversation(from, profileName);
    ...
    switch (buttonId) {
      ...
      case 'report_issue':
        conversation.state = ConversationState.reportIssue;
        await sendTextMessage(
            from, 'Please describe the issue you\'d like to report.');
        break;

      case 'main_menu':
        conversation.state = ConversationState.initial;
        await sendInteractiveMessage(from, 'How can I help you today?', [
          WhatsAppButton(id: 'track_bus', title: '🚌 Track Bus'),
          WhatsAppButton(id: 'schedule', title: '📅 Check Schedule'),
          WhatsAppButton(id: 'report_issue', title: '⚠️ Report Issue'),
        ]);
  ...
  Future<WhatsAppResponse> _handleLocationMessage(
      String from, Map<String, dynamic> location, String profileName) async {
  ...
  BotConversation _getOrCreateConversation(
  ...
  Future<void> sendBusArrivalNotification(String phoneNumber, String childName,
      String stopName, int minutesAway) async {
  ...
  Future<void> sendEmergencyAlert(String phoneNumber, String message,
      double? latitude, double? longitude) async {
    await sendTextMessage(phoneNumber, '🚨 EMERGENCY ALERT 🚨\n\n$message');

    if (latitude != null && longitude != null) {
      await sendLocationMessage(phoneNumber, latitude, longitude,
          'Emergency Location', 'Current bus location');
    }
  }

  Future<void> sendRouteDelayNotification(String phoneNumber, String routeName,
      int delayMinutes, String reason) async {
